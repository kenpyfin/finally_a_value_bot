...
#[derive(Error, Debug)]
#[allow(dead_code)]
pub enum FinallyAValueBotError {
...
mod tests {
    ...
    fn test_error_display_messages() {
        let e = FinallyAValueBotError::LlmApi("bad request".into());
        assert_eq!(e.to_string(), "LLM API error: bad request");

        let e = FinallyAValueBotError::RateLimited;
        assert_eq!(e.to_string(), "Rate limited, retry after backoff");

        let e = FinallyAValueBotError::ToolExecution("tool failed".into());
        assert_eq!(e.to_string(), "Tool execution error: tool failed");

        let e = FinallyAValueBotError::Config("missing key".into());
        assert_eq!(e.to_string(), "Config error: missing key");

        let e = FinallyAValueBotError::MaxIterations(25);
    ...
    fn test_error_from_io() {
        ...
        let e: FinallyAValueBotError = io_err.into();
    ...
    fn test_error_from_json() {
        ...
        let e: FinallyAValueBotError = json_err.into();

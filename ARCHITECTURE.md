...
Three engines are selectable in Settings → Agent engine, **per persona** (`personas.agent_engine_override`; empty inherits the global `AGENT_ENGINE` default):

- **Classic** — heuristic tool loop with optional Plan/Execute/Synthesize multi-model phases (`advance_phase` in `src/multimodel.rs`).
- **Deterministic** — structured pipeline in `src/agent_pipeline/`: cloud intent JSON → clarification gate → vault SOP or ephemeral plan → per-step local execution (retry + cloud escalation) → cloud synthesis → shared PDQE delivery. Dispatched from `process_with_agent_with_events` when the resolved persona engine is `Deterministic`. Shared bootstrap: `prepare_agent_run` in `src/channels/agent_run_prep.rs`.
- **Cursor** — delegates the full turn to a local **Cursor SDK sidecar** (`scripts/cursor-sdk-runner.mjs`, `@cursor/sdk` in-process), **auto-started on bot boot** on native installs (`CURSOR_SDK_AUTO_START`, default true). Rust flattens `prepare_agent_run` context into a prompt, streams assistant text from NDJSON, and finishes via `pipeline_finish_turn`. Each user message is a **new Cursor session** that is disposed when the reply is delivered (no on-disk resume store, no pooled `agent_id`). Recoverable sidecar errors return a notice instead of Classic. Requires `CURSOR_API_KEY` in repo-root `.env` (passed to sidecar subprocess; not stored in SQLite). Rollback: `CURSOR_SDK_RUNNER_SCRIPT=scripts/cursor-sdk-runner.py`.
